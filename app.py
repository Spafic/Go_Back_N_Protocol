        ...
        for j in range(i, min(i + window_size, number_of_frames + 1)):
            ...
            if random_result:
                ...
                if j + window_size <= number_of_frames:
                    ...
                    print(
                        f"Sending Frame {j + window_size}: Data: {next_frame_data}, Binary Representation: {convert_to_8bit_binary(next_frame_data)}")
                    total_sent_frames += 1
                ...
                for k in range(j, min(j + window_size, number_of_frames + 1)):
                    ...
                    print(
                        f"Resending Frame {k}: Data: {frame_data}, Binary Representation: {convert_to_8bit_binary(frame_data)}")
                    total_sent_frames += 1
...
def main():
    ...
    if len(message) % 2 != 0:
        message += '\0'  # Zero-extend if the length is odd
